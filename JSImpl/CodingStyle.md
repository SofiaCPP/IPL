...
When initializing an object, place a space before the leading brace as well as
between the braces and their content.
...
An else statement should go on the next line as a preceding close brace if one
is present.
...
One-line control clauses should not use braces unless comments are included or a
single statement spans multiple lines.

## Names

Use CamelCase. Capitalize the first letter, including all letters in an acronym,
in a class, function, struct, protocol, or namespace name.  Lower-case the first
letter, including all letters in a local variable.  Use prefix 'm_' for class
members and CamelCase without prefix for struct members.  Use CamelCase for
static class and struct members.
...
initializer syntax. Each member (and superclass) should be indented on a
