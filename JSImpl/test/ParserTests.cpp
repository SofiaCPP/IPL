...
TEST(Parser, VariableDeclaration)
{
	...
	ASSERT_TRUE(std::get<double>(*i.ModifyVariable("s")) == 0.0);
}
...
TEST(Parser, Assignment)
{
	...
	i.Run(expr.get());
	ASSERT_TRUE(i.HasVariable("s"));
	ASSERT_TRUE(std::get<double>(*i.ModifyVariable("s")) == 100);
}
...
TEST(Parser, VariableDeclarationAdditionOfLiterals)
{
	...
	i.Run(expr.get());
	ASSERT_TRUE(i.HasVariable("s"));
	ASSERT_DOUBLE_EQ(std::get<double>(*i.ModifyVariable("s")), 11.0);
}
...
TEST(Parser, VariableDeclarationMultiplicationOfLiterals)
{
	...
	i.Run(expr.get());
	ASSERT_TRUE(i.HasVariable("s"));
	ASSERT_DOUBLE_EQ(std::get<double>(*i.ModifyVariable("s")), 30.0);
}
...
TEST(Parser, Unary)
{
	...
	ASSERT_TRUE(i.HasVariable("a"));
	ASSERT_TRUE(i.HasVariable("b"));
	ASSERT_DOUBLE_EQ(std::get<double>(*i.ModifyVariable("a")), 6.0);
	ASSERT_DOUBLE_EQ(std::get<double>(*i.ModifyVariable("b")), 5.0);
	ASSERT_DOUBLE_EQ(std::get<double>(*i.ModifyVariable("minusB")), -5.0);
	ASSERT_DOUBLE_EQ(std::get<double>(*i.ModifyVariable("plusB")),  +5.0);
}
...
{
	{
		...
		ASSERT_DOUBLE_EQ(std::get<double>(*i.ModifyVariable("i")), 45.0);
	}
	{
		IPLVector<Token> tokens = Tokenize("var i = 0; for (var j = 0; j < 10; j++) i = i + j;").tokens;
		auto expr = Parse(tokens);
